  ...
  return new Promise((resolve, reject) => {
      ...
      if (message.type === 'result' && message.is_error === true) {
        const result =
          typeof message.result === 'string'
            ? message.result
            : `${options.adapter.displayName} 原生上下文整理失败`;
    ...
    child.once('close', (code) => {
      ...
      if (code !== 0) {
        fail(
          new Error(
            stderr.trim() || `${options.adapter.displayName} 退出，状态码 ${code}`
          )
        );
        return;
      }
      if (!completed) {
        ...
        // Cursor /summarize 通常只返回 result，没有 Claude 的 compact_boundary。
        if (resultMessage) {
          settled = true;
          cleanup();
        ...
        fail(
          new Error(
            `${options.adapter.displayName} 没有返回上下文整理完成事件`
          )
        );
        return;
      }
