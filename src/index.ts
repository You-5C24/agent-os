...
for (const config of botConfigs) {
  console.log(
    `[Bot ${config.id.toUpperCase()}] default_cli=${
      config.defaultCliId
...
async function startConfiguredBot(
  ...
  const startedBot = startBot({
    ...
    onMessage: async (msg, bot) => {
      ...
      const ownerOpenId =
        collaboration?.ownerOpenId ??
        (botSender
          ? process.env.OWNER_OPEN_ID ?? msg.senderOpenId
          : msg.senderOpenId);
      const ownerUnionId =
        collaboration?.ownerUnionId ??
      ...
      console.log(
        `  mentions: ${
          msg.mentions
            .map((m) => `${m.key}=${m.name}(${m.openId})`)
            .join(', ') || '(无)'
        }`
      );
      console.log(
        `  [会话] ${isNew ? '新建' : '复用'} id=${session.id} status=${
          session.status
        }`
      );
      ...
      void execution
        .then(async (result) => {
          ...
          console.log(
            `[CLI] ${cliAdapter.id} 完成 session_id=${
              result.sessionId ?? '(无)'
            }`
          );
          if (!collaboration) {
