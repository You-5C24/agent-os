...
async function listCursorSessions(): Promise<CliSessionSummary[]> {
  // `agent ls` 目前只有交互式选择器，没有机器可读输出；不要误走 Codex app-server。
  return [];
}
...
export function listNativeCliSessions(
  ...
  if (options.adapter.id === 'cursor') return listCursorSessions();
