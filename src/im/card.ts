...
function formatContextUsage(
  ...
  return `当前上下文 ${formatCount(usedTokens)} / ${formatCount(
    windowTokens
  )}（${percentage}%）`;
}
  ...
  const elements: Record<string, unknown>[] = [
    {
      tag: 'markdown',
      content: `**${currentIcon}${
        progress?.current ?? options.detail
      }**${currentDetail}\n\n${meta}${context ? `\n_${context}_` : ''}${
        contextGrowth ? `\n_${contextGrowth}_` : ''
      }`,
    },
  ...
  return flow.answers
    .map((answer, index) =>
      [
        ...
        `${
          answer.source === 'agent' ? 'Agent 推荐' : '你的选择'
        }：${escapeFeishuMarkdown(answer.answer)}`,
      ].join('\n')
    )
    .join('\n\n');
}
  ...
  return {
    ...
    body: {
      ...
      elements: [
        {
          tag: 'markdown',
          content: `**${options.targetName}，请接手：${escapeFeishuMarkdown(
            options.objective
          )}**`,
        },
        {
      ...
      elements: [
        {
          ...
          content: [
            ...
            run.completedAt
              ? `**结束时间** ${escapeFeishuMarkdown(run.completedAt)}`
              : '',
  ...
  const memberElements = options.members.map((member) => {
    ...
    return {
      ...
      content: [
        ...
        `引擎：${escapeFeishuMarkdown(
          member.cliName
        )}　Skill：${escapeFeishuMarkdown(skills)}`,
      ].join('\n'),
    };
  });
