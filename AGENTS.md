...
以飞书话题群为操作界面、Claude Code 与 Cursor 为执行引擎的个人生产系统（Codex 仅作备用）。一个进程可以运行多个 bot，每个 bot 只处理明确发给自己的消息，并拥有独立的角色、默认引擎与话题会话。
...
- 每个 bot 的 `defaultCli` 可设为 `claude` 或 `cursor`（`codex` 仅作备用）
- 每个 bot 用 `workspace` 指定默认项目目录；话题里的 `/cd` 只覆盖当前会话
- 工作目录变化后必须清除旧 CLI 会话 ID，避免把旧项目上下文带进新目录
- Claude Code 与 Cursor 的模型服务使用各自的用户级配置，不把模型密钥写进项目 `.env`
