...
function requireConfig(): void {
  ...
  if (missing.length) {
    ...
    console.error(
      '[agent-os] 请先从开发环境复制 .env 和 config/bots.json 到当前目录'
    );
    process.exit(1);
  }
}
...
function localDoctor(): void {
  ...
  for (const [command, checkArgs, label] of checks) {
    const value = output([command, ...checkArgs]);
    const name = label ? `${label} (${command})` : command;
    console.log(`${name}: ${value || '未安装'}`);
  }
  console.log(`.env: ${existsSync('.env') ? '存在' : '缺失'}`);
  console.log(
    `config/bots.json: ${existsSync('config/bots.json') ? '存在' : '缺失'}`
  );
}
...
function localStart(): void {
  ...
  ensureBuild();
  const running =
    runCommandSync('pm2', ['describe', 'agent-os'], { stdio: 'ignore' })
      .status === 0;
  if (running) {
