...
export class TeamRegistry {
  ...
  constructor(readonly leaderBotId: string, configs: BotConfig[]) {
    ...
    for (const config of this.members) {
      for (const skill of config.skills) {
        const searchedPaths = [
          ...
          join(config.workspaceDir, '.cursor', 'skills', skill, 'SKILL.md'),
          join(homedir(), '.agents', 'skills', skill, 'SKILL.md'),
          join(homedir(), '.claude', 'skills', skill, 'SKILL.md'),
          join(homedir(), '.cursor', 'skills', skill, 'SKILL.md'),
